...
sub init {
    ...
    if(-f $glade_file) {
        ...
        #$self->{content_paned}->remove($self->{content_window}) if($self->{content_window});
        #$self->{content} = NightGun::Gui::ContentView->new();
        #$self->{content}->init(NightGun::App::VIEW);
        #$self->{content}->set_container($self->{content_paned});
#        $self->{content}->show;
        $self->{content}->{listener}->{status_changed}=sub {$self->statusbar_set(@_);};
